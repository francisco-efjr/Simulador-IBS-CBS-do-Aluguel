...
export default function Index() {
  ...
  useEffect(() => {
    Promise.all([getImoveis(), getInquilinos(), getContratos(), getReceitas()])
      .then(([ims, inqs, cts, recs]) => {
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {overviewCards.map((card, idx) => {
          ...
          return (
            <Card
              key={idx}
              className="border border-slate-200/80 bg-white transition-all hover:shadow-elevation hover:-translate-y-0.5"
            >
