...
export class EnquadramentoEngine {
  ...
  public static evaluate(
    ...
    if (landlord.personType === 'pj') {
      return {
        isTaxpayer: true,
        reason: 'Pessoa Jurídica é contribuinte obrigatória de IBS/CBS nas operações de locação e cessão de bens imóveis.',
    ...
    if (isTaxpayer) {
      ...
      if (!exceedsPropertyLimit) {
        missingConditions.push(`possui até ${params.pfPropertyThreshold} imóveis alugados (${propertyCount})`);
      }
      if (!exceedsIncomeLimit) {
        missingConditions.push(`receita anual (R$ ${annualIncome.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}) não atinge o teto de R$ ${params.pfAnnualIncomeThreshold.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`);
      }
