  ...
  public static generateReport(
    ...
    if (input.propertyType === 'residential') {
      ...
      findings.push({
        ...
        status: 'info',
        message:
          'Imóveis comerciais não fazem jus ao redutor social conforme estipulado no Art. 260 da LC 214/2025.',
        articleReference: LEGAL_REFERENCES.REAL_ESTATE_REGIME.socialDeductionArticle,
      })
    }
    ...
    if (input.landlord.personType === 'pf') {
      ...
      findings.push({
        ...
        status: 'passed',
        message:
          'Pessoas Jurídicas são contribuintes compulsórios do IBS e da CBS em operações imobiliárias.',
    ...
    findings.push({
      ...
      status: 'info',
      message:
        'Para emissão regular de notas fiscais de locação (NFS-e), o imóvel deve estar inscrito no Cadastro de Identificação de Bens Imóveis (CIB) do Sinter.',
