...
export class TaxCalculatorEngine {
  ...
  public static calculateContract(
    ...
    const transitionYear: TransitionYear = sanitizedInput.transitionYear ?? params.transitionYear ?? 2033;
    ...
    if (landlord.personType === 'pj' && enquadramento.isTaxpayer) {
      ...
      landlordCredits = {
        ...
        creditExplanation: landlord.personType === 'pf'
    ...
    const passedToTenantLandlordNet = FinancialMath.round(baseRent - landlordCredits.managementFeeAmount);
    ...
    const effectiveLandlordBurden = landlordCredits.hasRightToCredits ? landlordCredits.netTaxToPay : totalTaxDue;
    const absorbedLandlordNet = FinancialMath.round(baseRent - effectiveLandlordBurden - landlordCredits.managementFeeAmount);
    const absorbedTaxBurden = baseRent > 0 ? FinancialMath.round((effectiveLandlordBurden / baseRent) * 100) : 0;
    ...
    if (!enquadramento.isTaxpayer) {
      creditNote = 'Locador não é contribuinte de IBS/CBS. Nenhum crédito tributário é gerado para o locatário.';
      ...
      creditNote = 'Locatário é PJ optante pelo Simples Nacional. Aproveitamento de créditos sujeito a regras específicas.';
    } else {
      creditNote = 'Locatário PJ no regime regular aproveita 100% de crédito do IBS/CBS destacado na NFS-e, neutralizando o tributo.';
    }
