...
export class ValidationEngine {
   ...
   */
  public static validateLeaseInput(
    input: LeaseContractInput,
  ): ValidationOutput<LeaseContractInput> {
    ...
    const sanitized: LeaseContractInput = {
      ...
      landlord: {
        ...
        totalAnnualRentalIncome: Math.max(
          0,
          FinancialMath.round(landlord.totalAnnualRentalIncome || 0),
        ),
   ...
   */
  public static validatePortfolio(items: PortfolioItem[]): {
    isValid: boolean
    errors: ValidationErrorItem[]
    warnings: string[]
  } {
    ...
    if (!items || items.length === 0) {
      warnings.push(
        'A carteira está vazia. Adicione ao menos um imóvel para simular a apuração agregada.',
      )
