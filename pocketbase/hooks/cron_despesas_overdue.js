cronAdd('despesas_overdue_check', '0 2 * * *', function () {
  ...
  try {
    ...
    $app.logger().info('despesas overdue check completed', 'updated', updated)
  } catch (err) {
    $app.logger().error('despesas overdue cron failed', 'error', String(err))
  }
})
