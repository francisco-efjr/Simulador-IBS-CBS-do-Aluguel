...
describe('tg_proteger_privilegio', () => {
  ...
  // Achado: o passo "Primeiro administrador" de supabase/README.md e da
  // migração 06 manda rodar, no SQL Editor, um UPDATE em public.users. Ali não
  // há sessão (auth.uid() é nulo), eh_administrador() devolve false e o gatilho
  // recusa — o banco novo fica sem como ganhar o primeiro administrador sem
  // desligar o gatilho. Falta liberar quem não é `authenticated` (postgres,
  // service_role), por exemplo testando auth.role() / current_user.
  it.fails('o passo "Primeiro administrador" do README funciona no SQL Editor (sem sessão)', async () => {
