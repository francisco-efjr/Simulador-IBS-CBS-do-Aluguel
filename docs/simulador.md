...
> ⚠️ **As faixas do IRPF mudam por lei todo ano.** Elas vivem isoladas em [`src/core/domain/irpfTable.ts`](./src/core/domain/irpfTable.ts), versionadas por ano-calendário — nenhum outro arquivo do sistema contém alíquota de IRPF. Confira os valores contra a tabela oficial da Receita Federal antes de publicar cada exercício. O redutor de isenção ampliada de 2026 é modelado como decaimento linear entre o teto de isenção e o teto do redutor; se a norma regulamentar fixar outra curva, ajuste `exemptionRelief` nesse arquivo.
