...
| <a id="rn-qdr-03"></a>RN-QDR-03 | Ler o quadro é aberto a qualquer pessoa, até sem login (ambiente de teste); criar e editar história e atividade exige edição no módulo `quadro`. Ninguém exclui história — a que não vale mais volta ao Backlog com a observação; atividade pode ser excluída. Visitante sem login não escreve nada nem lê outra tabela | A história é a documentação do sistema; o que foi combinado não some. Leitura aberta: é por ela que a família acompanha a construção (decisão do dono, 23/09/2026 — rever quando sair do ambiente de teste) | PO + Dono | banco (RLS) + tela | [`quadro.test.ts`](../../supabase/tests/quadro.test.ts) |
